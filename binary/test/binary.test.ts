import { ModelDescription, SerializedOCircuitConfig } from "core";
...
const OCIRCUIT_GLOBAL_DIR = path.join(__dirname, "..", ".ocircuit");
if (fs.existsSync(OCIRCUIT_GLOBAL_DIR)) {
  fs.rmSync(OCIRCUIT_GLOBAL_DIR, { recursive: true, force: true });
}
fs.mkdirSync(OCIRCUIT_GLOBAL_DIR);
...
describe("Test Suite", () => {
  ...
  beforeAll(async () => {
    ...
    const binaryPath = path.join(binaryDir, `ocircuit-binary${exe}`);
    const expectedItems = [
      `ocircuit-binary${exe}`,
      `rg${exe}`,
      "index.node",
    ...
    if (USE_TCP) {
      ...
      try {
        subprocess = spawn(binaryPath, {
          env: { ...process.env, OCIRCUIT_GLOBAL_DIR },
        });
  ...
  it("should create .ocircuit directory at the specified location with expected files", async () => {
    expect(fs.existsSync(OCIRCUIT_GLOBAL_DIR)).toBe(true);
    ...
    const missingFiles = expectedFiles.filter((file) => {
      const filePath = path.join(OCIRCUIT_GLOBAL_DIR, file);
  ...
  it("should add and delete a model from config.json", async () => {
    const model: SerializedOCircuitConfig["models"][number] = {
      title: "Test Model",
      provider: "openai",
      model: "gpt-3.5-turbo",
