...
describe("generateRepoMap", () => {
  ...
  it("should generate a repo map with signatures included", async () => {
    ...
    jest
      ...
      .mockImplementation(async (dirs, offset, limit) => {
        ...
        return {
          groupedByUri: {
            [joinPathsToUri(TEST_DIR, "file1.js")]: [
              "function foo() {}",
              "function bar() {}",
            ],
            [joinPathsToUri(TEST_DIR, "subdir/file2.py")]: [
              "def foo():",
              "def bar():",
            ],
          },
  ...
  it("should generate a repo map without signatures when includeSignatures is false", async () => {
    ...
    const expectedContent =
      ...
      "classes, methods, or functions in the file.\n\n" +
      "file1.js\n" +
      // No trailing newline: the includeSignatures=false path writes all
      // uris joined by "\n" with no trailing terminator (unlike the
      // includeSignatures=true path, whose per-file blocks each end in
      // "\n\n").
      "subdir/file2.py";
  ...
  it("should handle file read errors gracefully", async () => {
    ...
    jest
      ...
      .mockImplementation(async (dirs, offset, limit) => {
        ...
        return {
          groupedByUri: {
            [joinPathsToUri(TEST_DIR, "file1.js")]: ["function foo() {}"],
            [joinPathsToUri(TEST_DIR, "subdir/file2.py")]: ["def foo():"],
          },
          hasMoreSnippets: false,
          hasMoreUris: false,
        };
    ...
    // Mock testIde.readFile (the method generateRepoMap actually calls) to
    // throw an error when reading file1.js
    const realReadFile = testIde.readFile.bind(testIde);
    jest.spyOn(testIde, "readFile").mockImplementation((uri: string) => {
      if (uri === joinPathsToUri(TEST_DIR, "file1.js")) {
    ...
    const expectedContent =
      ...
      "subdir/file2.py:\n" +
      "\tdef foo():\n\n" +
      // No trailing newline: the "remaining uris without snippets" write
      // also joins with "\n" and adds no trailing terminator.
      "file1.js"; // file1.js is added in writeRemainingPaths
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      expect.stringContaining(
        "Failed to read file:\n" +
          `  Uri: ${joinPathsToUri(TEST_DIR, "file1.js")}\n` +
