import { markdownToRule } from "@opencircuit/config-yaml";
...
vi.mock("@opencircuit/config-yaml", () => ({
...
describe("loadCodebaseRules", () => {
  ...
  const mockRuleContent: Record<string, string> = {
    ...
    "file:///workspace/.ocircuit/rules.md":
  ...
  const mockConvertedRules: Record<string, any> = {
    ...
    "file:///workspace/.ocircuit/rules.md": {
      ...
      source: "colocated-markdown",
      sourceFile: "file:///workspace/.ocircuit/rules.md",
    },
  };
  ...
  it("should load rules from all rules.md files in the workspace", async () => {
    ...
    expect(rules).toContainEqual(
      mockConvertedRules["file:///workspace/.ocircuit/rules.md"],
    );
