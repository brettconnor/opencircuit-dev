import { ConfigResult, ConfigValidationError } from "@opencircuit/config-yaml";
...
type ConfigUpdateFunction = (payload: ConfigResult<OCircuitConfig>) => void;
...
export class ConfigHandler {
  ...
  async getLocalProfiles(options: LoadAssistantFilesOptions) {
    /**
     * Users can define as many local agents as they want in a `.ocircuit/agents` (or previous .ocircuit/assistants) folder
     */
    ...
    if (options.includeWorkspace) {
      ...
      const allFiles = (
        await Promise.all([
          getAllDotOCircuitDefinitionFiles(this.ide, yamlOptions, "assistants"),
          getAllDotOCircuitDefinitionFiles(this.ide, yamlOptions, "agents"),
          getAllDotOCircuitDefinitionFiles(this.ide, yamlOptions, "configs"),
        ])
      ).flat();
  ...
  private notifyConfigListeners(result: ConfigResult<OCircuitConfig>) {
  ...
  async getSerializedConfig(): Promise<
    ConfigResult<BrowserSerializedOCircuitConfig>
  > {
  ...
  async loadConfig(): Promise<ConfigResult<OCircuitConfig>> {
    await this.isInitialized;
    if (!this.currentProfile) {
      return {
        config: undefined,
