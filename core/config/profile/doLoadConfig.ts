...
import { getWorkspaceOCircuitRuleDotFiles } from "../getWorkspaceOCircuitRuleDotFiles";
import { loadOCircuitConfigFromJson } from "../load";
...
import { loadOCircuitConfigFromYaml } from "../yaml/loadYaml";

async function loadRules(ide: IDE) {
  const rules: RuleWithSource[] = [];
  const errors = [];

  // Add rules from .ocircuitrules files
  const { rules: yamlRules, errors: ocircuitRulesErrors } =
    await getWorkspaceOCircuitRuleDotFiles(ide);
  rules.unshift(...yamlRules);
  errors.push(...ocircuitRulesErrors);

  // Add rules from markdown files in .ocircuit/rules
...
export default async function doLoadConfig(options: {
  ...
  overrideConfigJson?: SerializedOCircuitConfig;
  ...
  let newConfig: OCircuitConfig | undefined;
  ...
  if (
    ...
    const result = await loadOCircuitConfigFromYaml({
      ide,
      ideSettings,
      ideInfo,
      uniqueId,
    ...
    const result = await loadOCircuitConfigFromJson(
      ide,
      ideSettings,
      ideInfo,
      uniqueId,
