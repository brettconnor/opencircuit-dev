...
import { IOCircuitServerClient } from "../../ocircuitServer/interface.js";
...
export class ChunkCodebaseIndex implements CodebaseIndex {
  ...
  constructor(
    ...
    private readonly ocircuitServerClient: IOCircuitServerClient,
  ...
  async *update(
    ...
    if (this.ocircuitServerClient.connected) {
      try {
        const keys = results.compute.map(({ cacheKey }) => cacheKey);
        const resp = await this.ocircuitServerClient.getFromIndexCache(
          keys,
          "chunks",
          repoName,
        );
