...
import { getOCircuitGlobalPath, readAllGlobalPromptFiles } from "../util/paths";
...
export async function getAllPromptFiles(
  ...
  // Also read from ~/.ocircuit/prompts and ~/.ocircuit/rules
  promptFiles.push(...readAllGlobalPromptFiles());

  const promptFilesFromRulesDirectory = readAllGlobalPromptFiles(
    path.join(getOCircuitGlobalPath(), RULES_DIR_NAME),
  );
