...
description: "Comprehensive guide to the config.yaml format used by Open Circuit for building custom coding agents. Learn how to define models, context providers, rules, prompts, and more using YAML configuration."
---

## Introduction

Open Circuit Agents are defined using the `config.yaml` specification.
...
Learn how to work with Open Circuit's configuration system, including using hub models, rules, and tools, creating local configurations, and organizing your setup.
</Card>
...
- `capabilities`: Array of strings denoting model capabilities, which will overwrite Open Circuit's autodetection based on provider and model. See the [Model Capabilities guide](/customize/deep-dives/model-capabilities) for detailed information. Supported capabilities include:
  ...
  Open Circuit automatically detects these capabilities for most models, but you can override this when using custom deployments or if autodetection isn't working correctly.
...
The [Model Context Protocol](https://modelcontextprotocol.io/introduction) is a standard proposed by Anthropic to unify prompts, context, and tool use. Open Circuit supports any MCP server with the MCP context provider.
...
data:
  - name: Local Data Bank
    destination: file:///Users/dallin/Documents/code/open-circuit-dev/open-circuit-extras/external-data
    schema: 0.2.0
    level: all
