---
title: "Code Review Bot with Open Circuit and GitHub Actions"
description: "Set up automated, context-aware pull request reviews using Open Circuit CLI in GitHub Actions - privacy-first with custom rules"
...
<CardGroup cols={3}>
  <Card title="Privacy-First" icon="shield">
    All logs and processing happen in your runner: Open Circuit CLI runs in GitHub Actions → code to your LLM provider (OpenAI, Anthropic, etc.). No hosted Open Circuit service reads your code.
  ...
  <Card title="Context Awareness" icon="robot">
    Leverage Open Circuit's AI agent for intelligent, context-aware reviews with full control over your configuration.
...
- A Open Circuit account
  - Read: [Understanding Configs](/guides/understanding-configs)
- A Open Circuit API key from [docs.open-circuit.dev/settings/api-keys](//settings/api-keys)
- Open Circuit assistant configured for code reviews (or use our recommended default)

<Info>
  **Want to customize the review bot?**
  
  You can remix the default review bot configuration at [docs.open-circuit.dev/open-circuit-dev/review-bot](//open-circuit-dev/review-bot) to create your own personalized version with custom prompts, rules, and behaviors.
</Info>
    ...
    steps:
      ...
      - name: Run Open Circuit Review
        ...
        run: |
          ...
          # Run Open Circuit CLI in headless mode
          cn --config open-circuit-dev/review-bot \
             -p "$PROMPT" \
      ...
      - name: Post Review Comment
        ...
        run: |
          ...
          ---
          *Powered by [Open Circuit](/) • Need a focused review? Comment `@review-bot check for [specific concern]`*
          EOF
...
5. **Open Circuit CLI Analyzes Code** - AI agent reviews the code with your rules
...
<Tabs>
  <Tab title="Use Your Own Open Circuit Config">
    
  By default, the workflow uses the `open-circuit-dev/review-bot` config optimized for code reviews. Replace `open-circuit-dev/review-bot` with your own config:
...
## Troubleshooting
<AccordionGroup>
<Accordion title="Open Circuit CLI not found">
- The workflow installs the CLI automatically, but ensure Node.js 20+ is available
- Check the "Install Open Circuit CLI" step logs for errors
</Accordion>
...
- Verify your Open Circuit config is accessible
- Check Open Circuit CLI logs in the workflow run
...
>
> ---
> *Powered by [Open Circuit](/) • Need a focused review? Comment `@review-bot check for security`*
...
5. **Create team config** - Set up a shared Open Circuit config for consistent reviews
...
<CardGroup cols={2}>
  ...
  <Card title="Open Circuit CLI Guide" icon="terminal" href="/guides/cli">
    Learn more about Open Circuit CLI capabilities
  </Card>
