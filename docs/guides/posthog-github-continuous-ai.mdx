...
sidebarTitle: "PostHog Analytics with Open Circuit CLI"
---
...
<Card title="What You'll Build" icon="robot">
  A fully automated workflow that uses Open Circuit CLI with the PostHog MCP to fetch analytics data, analyze user experience issues with AI, and automatically create GitHub
...
<Steps>
  ...
  <Step title="Add Required Secrets to Open Circuit CLI">
    Open Circuit CLI will securely store your API keys as secrets that can be referenced in prompts.
  </Step>
</Steps>

<Tip>
  Open Circuit CLI handles the complex API interactions - you just need to provide
  the right prompts!
</Tip>

## Step 1: Set Up Your Credentials

First, you'll need to gather your PostHog and GitHub API credentials and add them as secrets in Open Circuit CLI.
...
<Tabs>
  <Tab title="PostHog API Credentials">
    ...
    3. Name it "Open Circuit CLI Session Analysis"
    ...
    8. You'll also need your POSTHOG_AUTH_HEADER value, which is simply `Bearer YOUR_API_KEY`
      ...
      <Info>
  **Open Circuit Secrets**: The `POSTHOG_AUTH_HEADER` secret should be stored in
  Open Circuit's secure secrets storage. This keeps your API key safe and the MCP
...
<Tab title= "Configure API Access in Open Circuit CLI">
...
<Tabs>
    ...
    **Perfect for:** Immediate results with optimized prompts and built-in debugging
    ...
    <Steps>
      <Step title="Add the Pre-Built Agent">
        ...
        ```bash
        cn --agent open-circuit-dev/posthog-continuous-ai-agent
        ```
  ...
  <Steps>
    <Step title="Add PostHog MCP to Open Circuit CLI">
    ...
    <Step title="Create Your Custom Prompts">
      Use this prompt with Open Circuit CLI to analyze PostHog data and create GitHub issues:
...
Here are practical examples of what you can build with PostHog MCP and Open Circuit CLI:
...
**Example Open Circuit CLI prompts:**
...
Consider enhancing your workflow with these advanced Open Circuit CLI prompts:
