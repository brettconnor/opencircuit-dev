---
title: "GitHub Issues and PRs with GitHub MCP and Open Circuit"
description: "Use Open Circuit and the GitHub MCP to list, summarize, and act on open issues and recently merged pull requests with natural language prompts."
sidebarTitle: "GitHub Issues with Open Circuit"
---
...
<Card title="What You'll Build" icon="github">
  A GitHub workflow assistant that uses Open Circuit with the GitHub MCP to:
...
<Card title="Fastest Path to Success" icon="rocket">
  Use the GitHub MCP with Open Circuit CLI for quick setup.
</Card>
  ...
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Load the Pre-Built Agent">
        Run:
        ```bash
        cn --agent open-circuit-dev/github-project-manager-agent
        ```
...
<Accordion title="Agent Requirements">
    To use GitHub MCP with Open Circuit CLI, you need either:
...
- `CONTINUE_API_KEY`: From [docs.open-circuit.dev/settings/api-keys](//settings/api-keys)
  ...
  report:
    ...
    steps:
      ...
      - name: Post PR Context Comment
        ...
        run: |
          REPORT=$(        cn --agent open-circuit-dev/github-project-manager-agent \
...
<Info>
  Use `open-circuit-dev/github-project-manager-agent` consistently throughout your automation as the recommended agent configuration.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="GitHub Project Manager Agent" icon="robot" href="//guides/github-mcp-continue-cookbook">
