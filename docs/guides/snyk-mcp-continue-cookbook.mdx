---
title: "Automated Security Scanning with Snyk MCP and Open Circuit"
...
sidebarTitle: "Snyk Security Scanning with Open Circuit"
---
...
<Card title="What You'll Build" icon="shield-halved">
  An automated security scanning system that uses Open Circuit's AI agent with Snyk
...
<iframe
  ...
  title="Snyk MCP Open Circuit Cookbook Demo"
  frameborder="0"
...
<Note>
  **Important**: The Snyk MCP requires the Snyk CLI to be authenticated locally. Run `snyk auth` to authenticate before using the Open Circuit agent with Snyk MCP.
</Note>
 <Warning>
  ...
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Load the Pre-Built Agent">
        ...
        ```bash
        cn --agent open-circuit-dev/snyk-continuous-ai-agent
        ```
...
<Steps>
  <Step title="Add Snyk MCP to Open Circuit CLI">
    ...
    <Tip>
      **Installation methods:**
      ...
      Once installed, Snyk MCP tools become available to your Open Circuit agent for all prompts.
    </Tip>

    <Info>
      The MCP will request authentication and folder trust permissions when first used.
      This is handled automatically by the Open Circuit agent.
    </Info>
...
Now you can use natural language prompts to run comprehensive security scans. The Open Circuit agent automatically calls the appropriate Snyk MCP tools.
    ...
    steps:
      ...
      - name: Generate AI Mitigation Suggestions
        ...
        run: |
          ...
          # Create a summary of findings for Open Circuit CLI
          ...
          if [ -n "$FINDINGS_SUMMARY" ]; then
            ...
            # Use Open Circuit CLI to generate mitigation suggestions
            ...
            cn --agent open-circuit-dev/snyk-continuous-ai-agent -p "$PROMPT" --auto > mitigation-suggestions.md || {
      ...
      - name: Post Mitigation Summary to PR
        ...
        run: |
          ...
          **Scan Details:**
          ...
          - 🤖 Generated with Open Circuit CLI + Snyk
...
<Info>
  **About SNYK_TOKEN**: The workflow uses the SNYK_TOKEN in two ways:
  ...
  2. **Open Circuit CLI access** - Available as an environment variable when Open Circuit generates AI mitigation suggestions
...
<Info>
  ...
  - **AI Mitigation**: Uses Open Circuit CLI to generate actionable mitigation steps
...
Implement automated security policies using Open Circuit's rule system. See the [Rules deep dive](/customize/deep-dives/rules) for authoring tips.
