...
description: "Optimize web performance with A/B testing, automated monitoring, and data-driven improvements using Netlify MCP and Open Circuit."
  ...
  <Tab title="⚡ Quick Start (Recommended)">
    ...
    <Steps>
      <Step title="Add the Pre-Built Agent">
        ...
        ```bash
        cn --agent open-circuit-dev/netlify-continuous-ai-agent
        ```
...
jobs:
  performance:
    ...
    steps:
      ...
      - name: Authenticate Open Circuit CLI
        env:
          CONTINUE_API_KEY: ${{ secrets.CONTINUE_API_KEY }}
        run: |
          cn auth login --api-key "$CONTINUE_API_KEY"
          echo "✅ Open Circuit CLI authenticated"
...
- Install the Netlify MCP in your Open Circuit configuration
...
- [Open Circuit Performance Guides](//guides)
