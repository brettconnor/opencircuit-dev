---
title: "Developer & Team Workflows with Notion + Open Circuit CLI"
description: "Use Open Circuit CLI with Notion to generate docs, manage tasks, and automate project workflows – all through natural-language prompts."
sidebarTitle: "Notion with Open Circuit"
---
...
- Use natural language to connect to the Notion API directly with Open Circuit CLI for powerful automation
...
- [Open Circuit CLI](//cli/quickstart) installed
- A [Notion workspace](https://notion.so) with Editor (or higher) access
- Node.js 18+ installed locally
- [Open Circuit account](/)

<Warning>

  **Agent usage requires credits** – create a Open Circuit API key at
  [docs.open-circuit.dev/settings/api-keys](//settings/api-keys)
...
<Steps>
  ...
  <Step title="Create Notion Integration & Get API Key">
    ...
    2. Click **+ New integration** → give it a name (e.g. "Open Circuit Integration")
...
## Running Open Circuit CLI with Notion API

<Card title="🚀 Choose Your Interface" icon="zap">

  Open Circuit CLI offers two powerful modes for Notion automation:
...
<Accordion title="API Connection Notes">
  ...
  - Environment variable `NOTION_API_KEY` must be set before running Open Circuit CLI
  - Open Circuit automatically uses the API key to authenticate with Notion
  - No need for manual curl commands - just reference "the API key stored in this session"
  - For complex workflows, Open Circuit maintains the API connection throughout
...
Working example that demonstrates the power of Open Circuit with Notion API:
...
<Accordion title="Common Issues and Solutions">
  ...
  **Connection Issues:**
  - Verify Open Circuit CLI has internet access
