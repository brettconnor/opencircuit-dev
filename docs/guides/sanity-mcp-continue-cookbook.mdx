---
title: "Content Management with Sanity MCP and Open Circuit"
...
sidebarTitle: "Sanity CMS with Open Circuit"
---
...
<Card title="What You'll Build" icon="database">
  An AI-powered content management system workflow that uses Open Circuit's AI agent with Sanity
...
<Step title="Install Sanity CLI">
 ...
 <Warning>
  To use agents in headless mode, you need a [Open Circuit API key](//settings/api-keys) and proper environment variable configuration.
  </Warning>
  ...
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Load the Pre-Built Agent">
        ...
        ```bash
        cn --agent open-circuit-dev/sanity-assistant-agent
        ```

        This agent includes:
        - **[Sanity MCP](//sanity/sanity-mcp)** pre-configured and ready to use
...
<Steps>
  <Step title="Add Sanity MCP to Open Circuit CLI">
    ...
    <Tip>
      **Installation methods:**
      ...
      Once installed, Sanity MCP tools become available to your Open Circuit agent for all prompts.
    </Tip>
...
Now you can use natural language prompts to manage your Sanity content and schemas. The Open Circuit agent automatically calls the appropriate Sanity MCP tools.
...
<Info>
  The workflow uses the [Sanity Assistant Agent](//open-circuit-dev/sanity-assistant-agent) with environment variable authentication via [Sanity MCP Config](//sanity/sanity-mcp-config). This enables headless mode operation without OAuth browser authentication.
</Info>

### Create Workflow File

This workflow automatically validates your Sanity schemas and content on pull requests using the Open Circuit CLI in [headless mode](/cli/headless-mode). It checks schema integrity, validates content relationships, and posts a summary report as a PR comment.
  ...
  validate-content:
    ...
    steps:
      ...
      - name: Comment Report on PR
        ...
        run: |
          REPORT=$(          cn --agent open-circuit-dev/sanity-assistant-agent \
...
<Info>
  Environment variables enable the MCP to authenticate without OAuth browser prompts. The [Sanity MCP Config](//sanity/sanity-mcp-config) documentation provides detailed setup instructions for all required variables.
</Info>

## Content Management Best Practices

Implement automated content quality checks using Open Circuit's rule system. See the [Rules deep dive](/customize/deep-dives/rules) for authoring tips.
