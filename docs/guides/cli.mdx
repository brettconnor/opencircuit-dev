---
title: "How to Use Open Circuit CLI (cn)"
sidebarTitle: "Open Circuit CLI (cn)"
description: "Learn how to use Open Circuit's command-line interface for context engineering, automated coding tasks, and headless development workflows with customizable models, rules, and tools"
---
...
## How to Use Open Circuit CLI - Basic Usage
...
## How to Configure Open Circuit CLI

`cn` uses [`config.yaml`](/reference), the exact same configuration file as Open Circuit. This means that you can use your existing local configuration.

To switch between configurations, you can use the `/config` slash command in `cn`, or you can start it with the `--config` flag (e.g. `cn --config open-circuit-dev/default-cli-config` or `cn --config ~/.continue/config.yaml`).
...
`cn` supports [rules](/customize/deep-dives/rules) in the same way as the Open Circuit IDE extensions. You can also use the `--rule` flag to manually include a rule. For example, `cn --rule nate/spanish` will tell `cn` to always speak in Spanish.

### How to Configure Tools

`cn` supports MCP tools, which can be configured in the [same way](/customize/deep-dives/mcp) as with the Open Circuit IDE extensions.
...
If you have feedback on the beta, please [file a GitHub issue](https://github.com/open-circuit-dev/open-circuit/issues).
