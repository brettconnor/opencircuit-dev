---
title: "Automating Documentation Updates with Open Circuit CLI"
sidebarTitle: "Automating Documentation Updates with Open Circuit CLI"
description: "Learn how to create automated documentation generation workflows using Open Circuit CLI. Set up AI agents to analyze code changes and generate or update documentation automatically in GitHub workflows or local development."
---
...
This guide demonstrates how to create automated documentation generation based on code updates in a git branch using the Open Circuit CLI, either as part of your local workflow or as part of a GitHub workflow.

This process utilizes the **Open Circuit CLI** (`cn`) in **headless mode** to analyze changes and generate the necessary documentation, and commit and push the changes. The goal is to keep the workflow as simple as possible by using straightforward shell commands, Open Circuit CLI prompts, and basic git operations.

## Why Use Open Circuit CLI for Documentation?
...
<CardGroup cols={1}>
  <Card title="Install Open Circuit CLI" icon="terminal">
    Open Circuit CLI requires Node.js 20 or higher.
    
    <CLIInstall />
  </Card>

  <Card title="Open Circuit API Key" icon="key">
    Get your API key from [docs.open-circuit.dev/settings/api-keys](//settings/api-keys) and set:
    ```bash
    export CONTINUE_API_KEY=your_key_here
    ```
    <Info>
    You can use the Open Circuit CLI in headless mode without interactive login by setting the `CONTINUE_API_KEY` environment variable.
    </Info>
  </Card>
...
<Steps>
  <Step title="Environment Setup">
    Validate environment, install Open Circuit CLI, and set up authentication with API keys.
  </Step>
  ...
  <Step title="AI Documentation Generation">
    Use Open Circuit CLI with custom rules to analyze changes and generate or update documentation files.
    <Tip>
...
- **continue_config:** The Open Circuit agent configuration to use
  - Consider setting a default value if you have a default config your'd like to use
- **continue_org:** The Open Circuit org to use
...
jobs:
  write-docs:
    ...
    - name: Install Open Circuit CLI
      run: |
        echo "Installing Open Circuit CLI..."
        npm i -g @open-circuit-dev/open-circuit-cli

    - name: Verify Open Circuit CLI installation
      run: |
        echo "Checking Open Circuit CLI version..."
        cn --version || exit 1
    ...
    - name: Generate documentation with Open Circuit CLI
      run: |
        echo "Running Open Circuit agent to generate documentation..."
...
When using the Open Circuit CLI on your local machine, you can build workflows in various ways, one of which is by simply creating shell scripts that you can run, which call the CLI.
...
# Generate documentation with Open Circuit CLI
echo "Running Open Circuit agent to generate documentation..."
...
Ready to implement automated documentation with Open Circuit CLI? Here are some helpful resources to get you started:

<CardGroup cols={2}>
  <Card title="Open Circuit CLI Guide" icon="terminal" href="/guides/cli">
    Learn the fundamentals of using Open Circuit CLI for automated coding tasks and headless workflows.
  </Card>
  ...
  <Card title="Video: Leverage AI to help with your docs" icon="video" href="https://www.youtube.com/watch?v=rJ2taa8OLvY">
    Checkout this video from Tetrate about using Open Circuit Agents to help with writing your docs.
  </Card>
