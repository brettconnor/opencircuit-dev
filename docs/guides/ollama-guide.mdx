---
title: "Using Ollama with Open Circuit: A Developer's Guide"
description: "Complete guide to setting up Ollama with Open Circuit for local AI development. Learn installation, configuration, model selection, performance optimization, and troubleshooting for privacy-focused offline coding assistance"
---
...
- Open Circuit extension installed
...
## How to Configure Ollama with Open Circuit

There are multiple ways to configure Ollama models in Open Circuit:
...
<Warning>
  **Important**: Blocks only provide configuration - you still need to pull
  the model locally. The block `ollama/deepseek-r1-32b` configures Open Circuit
  to use `model: deepseek-r1:32b`, but the actual model must be installed:
  ```bash
  # Check what the block expects (view on docs.open-circuit.dev)
...
Open Circuit can automatically detect available Ollama models. You can configure this in your YAML:
...
3. Open Circuit will scan for available Ollama models
...
<Note>
  ...
  available models. When set to `AUTODETECT`, Open Circuit will dynamically populate
...
Open Circuit may use a higher default context length than other tools. Reduce `contextLength` in your config (e.g., to 2048), or try a smaller model. See [Ollama provider troubleshooting](/customize/model-providers/top-level/ollama#troubleshooting) for details.
...
async def create_user(user: User):
    # Open Circuit will help complete this implementation
...
Use Open Circuit with Ollama to:
...
Ollama with Open Circuit provides a powerful local development environment for AI-assisted coding. You now have complete control over your AI models, ensuring privacy and enabling offline development workflows.

---

_This guide is based on Ollama v0.11.x and Open Circuit v1.1.x. Please check for updates regularly._
