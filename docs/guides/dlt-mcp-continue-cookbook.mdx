---
title: "Building Data Pipelines with dlt MCP and Open Circuit"
...
sidebarTitle: "dlt Data Pipelines with Open Circuit"
---
...
<Card title="What You'll Build" icon="database">
  An AI-powered data pipeline development system that uses Open Circuit's AI agent with dlt
...
<Card title="🚀 Fastest Path to Success" icon="zap">
  Skip the manual setup and use our pre-built [dlt Agent](//open-circuit-dev/dlt-agent) that includes
  ...
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Load the Pre-Built Agent">
        ...
        ```bash
        cn --agent open-circuit-dev/dlt-agent
        ```
    ...
    <Info>
      **Why Use the Agent?** The pre-built [dlt Agent](//open-circuit-dev/dlt-agent) provides consistent pipeline development workflows and handles MCP configuration automatically, making it easier to get started with AI-powered data engineering. You can customize this agent later to fit your team's specific workflow.
    </Info>
...
<Steps>
  <Step title="Add dlt MCP to Open Circuit CLI">
    ...
    <Tip>
      **Installation methods:**
      ...
      Once installed, dlt MCP tools become available to your Open Circuit agent for all prompts.
    </Tip>
...
<Accordion title="Agent Requirements">
    To use the pre-built [dlt Agent](//open-circuit-dev/dlt-agent), you need either:
...
<Card title="Understanding the Difference" icon="info-circle">
  ...
  **[dlt+ MCP](//dlthub/dlt-plus-mcp)** extends these capabilities with cloud-based features for production deployments:
  ...
  For local development and getting started, **[dlt MCP](//dlthub/dlt-mcp)** is the right choice. Consider **[dlt+ MCP](//dlthub/dlt-plus-mcp)** when you need production deployment features and team collaboration.
</Card>
...
Now you can use natural language prompts to develop and debug your dlt pipelines. The Open Circuit agent automatically calls the appropriate dlt MCP tools.

<Info>
  **Where to run these workflows:**
  - **IDE Extensions**: Use Open Circuit in VS Code, JetBrains, or other supported IDEs
...
## Continuous Data Pipelines with GitHub Actions
  ...
  This example demonstrates a **Continuous AI workflow** where data pipeline validation runs automatically in your CI/CD pipeline in headless mode using the [dlt Assistant agent](//dlthub/dlt-assistant). Consider customizing this agent to add your organization's specific validation rules.

### Add GitHub Secrets

Navigate to **Repository Settings → Secrets and variables → Actions** and add:

- `CONTINUE_API_KEY`: Your Open Circuit API key from [docs.open-circuit.dev/settings/api-keys](//settings/api-keys)
- Any required database credentials for your destination

<Info>
      The workflow uses the pre-built [dlt Agent](//open-circuit-dev/dlt-agent) with `--agent open-circuit-dev/dlt-agent`. This agent comes pre-configured with the dlt MCP and optimized rules for pipeline operations. You can customize the validation rules and prompts for your specific pipeline requirements.
</Info>

### Create Workflow File

This workflow automatically validates your dlt data pipelines on pull requests using the Open Circuit CLI in [headless mode](/cli/headless-mode). It inspects pipeline schemas, checks for errors, and posts a summary report as a PR comment. The workflow can also be triggered manually via `workflow_dispatch`.
  ...
  validate-pipeline:
    ...
    steps:
      ...
      - name: Install Open Circuit CLI
        run: |
          npm install -g @open-circuit-dev/open-circuit-cli
          echo "✅ Open Circuit CLI installed"
      ...
      - name: Comment Pipeline Report on PR
        ...
        run: |
          REPORT=$(cn --agent open-circuit-dev/dlt-agent \
...
Implement automated pipeline quality checks using Open Circuit's rule system. See the [Rules deep dive](/customize/deep-dives/rules) for authoring tips.
...
<CardGroup cols={2}>
  ...
  <Card title="Open Circuit Documentation" icon="book" href="/">
    Explore Open Circuit documentation and guides
  </Card>
  <Card
  ...
  >
    Learn about AI agents, MCP, and Open Circuit integration
  </Card>
  <Card
    title="MCP Concepts"
    icon="puzzle-piece"
