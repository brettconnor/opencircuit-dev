...
Run `cn` to start an interactive session. You get a prompt where you can type messages, reference files with `@`, and use slash commands. `cn` uses the same underlying agent as the Open Circuit IDE extensions.
...
| `/update` | Update the Open Circuit CLI |
...
When you're connected to a remote environment, Open Circuit also adds remote-only slash commands such as `/diff` and `/apply`.
...
- **Open Circuit** — approve this call
- **Open Circuit + don't ask again** — approve and save a policy rule to `~/.continue/permissions.yaml`
