...
Open Circuit CLI (`cn`) is a terminal-based coding agent. It can edit files, run commands, and work through multi-step tasks — the same agent that powers the Open Circuit IDE extensions, running in your terminal.
...
- A [Open Circuit](/) account, or an Anthropic API key.
...
On first launch you'll be asked to log in with [Open Circuit](/) or enter an Anthropic API key. After that, you're in a session and can start typing.

## Authentication

### Log in with Open Circuit

```bash
cn login
```

This opens your browser to authenticate with [Open Circuit](/). Once authenticated, `cn` can use your configured assistants, models, and MCP servers from the platform.
...
If you don't want to use Open Circuit, you can use an Anthropic API key directly. On first launch, `cn` will prompt you to choose between logging in with Open Circuit or entering an Anthropic API key.
