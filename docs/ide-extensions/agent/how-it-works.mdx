...
4. Open Circuit calls the tool using built-in functionality or the MCP server that offers that particular tool
5. Open Circuit sends the result back to the model
...
Open Circuit includes several built-in tools which provide the model access to IDE functionality.
