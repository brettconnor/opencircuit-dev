---
title: "Quick Start"
description: "Get started with Open Circuit's Agent mode to automatically implement code changes, fix bugs, and run commands using AI-powered tools that can modify your codebase based on natural language instructions"
---
...
<Info>
  ...
  model or provider doesn't support tools, or Open Circuit doesn't yet support tools
...
By default, Agent mode will ask permission when it wants to use a tool. Click `Open Circuit` to allow Agent mode to proceed with the tool call or `Cancel` to reject it.
