...
1. Open Circuit sends a special system message instructing the model to focus on understanding and planning
...
<Info>
  ...
  are not filtered by Open Circuit. Some MCP tools may have write capabilities that
...
<Info>
  ...
  the
  code](https://github.com/open-circuit-dev/open-circuit/blob/main/core/llm/defaultSystemMessages.ts).
</Info>
