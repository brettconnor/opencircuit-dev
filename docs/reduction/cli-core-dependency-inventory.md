...
| JetBrains metadata/contributor docs | Remove (Phase 2a, Batch 3 of 3)   | Owner has confirmed no plans to support the JetBrains plugin (see `docs/planning/phase2a-cut-jetbrains.md`); batches 1 (CI) and 2 (plugin source) removed the functional surface, this batch cleans up remaining references | Removed the "JetBrains Extension" secrets table from `BUILD_DEPENDENCIES.md` and the `jetbrains-release.yaml` mention from its `CI_GITHUB_TOKEN` row. Updated `README.md`'s intro capability line and removed its dead `### JetBrains` section (source link pointed at the now-deleted `extensions/intellij`); left the historical "final 2.0.0 release" sentence intact as an accurate past-tense record. Updated `CONTRIBUTING.md`'s `#### JetBrains` subsection to a short removal notice (superseding the prior Batch-G disclaimer that promised a future packaging-pipeline repair, since JetBrains is now fully cut rather than repaired) and removed a dead `GetTheme.kt` theme-color guideline. Removed the now-fully-obsolete `.continue/rules/intellij-plugin-test-execution.md` rule file (its glob matched only the now-deleted `extensions/intellij/**/*Test.kt`). Updated `.continue/rules/new-protocol-message.md` (removed the dead "Add to IntelliJ constants" step referencing a deleted Kotlin file) and `.continue/checks/update-continue-docs.md` (updated the "Context: Continue" capability sentence). Removed a stale "JetBrains integration" TODO line from `core/nextEdit/README.md`, updated `binary/README.md`'s "Debugging" section (previously instructed developers to use the now-deleted `CoreMessenger.kt`'s `useTcp` IntelliJ debug path), and removed the dead `extensions/intellij/build/**` search-exclude entry from `.vscode/settings.json`. Explicitly left untouched (out of scope): `.continue/rules/documentation-standards.md`'s general "VS Code and JetBrains when applicable" authoring guidance (conditional language already handles JetBrains' absence; user-facing `docs/`/`docs-site/` content is a separate docs-content decision per the phase2a plan), `TESTING.md`'s already-checked-off historical `[x]` test record, `extensions/vscode/config_schema.json`'s ~30 `x-intellij-html-description` JSON Schema keys (inert vestigial schema output, not contributor-doc metadata — flagged as a residual cleanup candidate for a future, narrowly-scoped experiment), and `packages/config-yaml/CHANGELOG.md` (historical, upstream-generated changelog). Re-verified: CLI build clean (12.69 MB bundle); both `boundary-check.mjs` and `runtime-boundary-check.mjs` pass with `violations: []` (4135 bundle inputs, unchanged) | All 3 Phase 2a experiment-order items complete pending merge; `extensions/vscode/config_schema.json`'s vestigial `x-intellij-html-description` keys flagged as a residual, out-of-scope cleanup candidate |
