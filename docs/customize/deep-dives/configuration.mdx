---
title: "How to Configure Open Circuit"
description: Learn how to access and manage Open Circuit configurations through local YAML files
keywords: [config, settings, customize]
sidebarTitle: "Configuration"
---

You can easily access your configuration from the Open Circuit Chat sidebar. Open the sidebar by pressing <kbd>cmd/ctrl</kbd> + <kbd>L</kbd> (VS Code) or <kbd>cmd/ctrl</kbd> + <kbd>J</kbd> (JetBrains) and click the Agent selector above the main chat input. Then, you can hover over an agent and click the `gear` icon.
...
When editing this file, you can see the available options suggested as you type, or check the reference below. When you save a config file from the IDE, Open Circuit will automatically refresh to take into account your changes. A config file is automatically created the first time you use Open Circuit, and always automatically generated with default values if it doesn't exist.
...
`config.yaml` or `config.json` can handle the vast majority of necessary configuration, so we recommend using it whenever possible. However, if you need to programmatically extend Open Circuit configuration, you can use a `config.ts` file, placed at `~/.continue/config.ts` (MacOS / Linux) or `%USERPROFILE%\.continue\config.ts` (Windows).
