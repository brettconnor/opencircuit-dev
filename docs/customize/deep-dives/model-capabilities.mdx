---
title: "How to Configure Model Capabilities in Open Circuit"
...
Open Circuit needs to know what features your models support to provide the best experience. This guide explains how model capabilities work and how to configure them.

## What Are Model Capabilities?

Model capabilities tell Open Circuit what features a model supports:
...
## How Open Circuit Detects Model Capabilities

Open Circuit uses a two-tier system for determining model capabilities:

### How Automatic Detection Works (Default)

Open Circuit automatically detects capabilities based on your provider and model name. For example:
...
- [toolSupport.ts](https://github.com/open-circuit-dev/open-circuit/blob/main/core/llm/toolSupport.ts) - Tool capability detection logic
- [@open-circuit-dev/llm-info](https://www.npmjs.com/package/@open-circuit-dev/llm-info) - Image support detection

### How to Configure Capabilities Manually

You can add capabilities to models that Open Circuit doesn't automatically detect in your `config.yaml`.

<Note>
  You cannot override autodetection - you can only add capabilities. Open Circuit
...
2. **Using newer models** - Open Circuit doesn't yet recognize a newly released model
...
Add tool support for a model that Open Circuit doesn't recognize:
...
<Warning>
  An empty capabilities array does not disable autodetection. Open Circuit will
...
3. **Keep Open Circuit updated** - Newer versions improve autodetection

Remember: Setting capabilities only adds to autodetection. Open Circuit will still use its built-in knowledge about your model in addition to your specified capabilities.

## Model Capability Support

This matrix shows which models support tool use and image input capabilities. Open Circuit auto-detects these capabilities, but you can override them if needed.
