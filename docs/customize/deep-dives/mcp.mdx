---
title: "How to Set Up Model Context Protocol (MCP) in Open Circuit"
...
## How to Set Up Open Circuit Documentation Search with MCP

You can set up an MCP server to search the Open Circuit documentation directly from your config. This is particularly useful for getting help with Open Circuit configuration and features.

For complete setup instructions, troubleshooting, and usage examples, see the [Open Circuit MCP Reference](/reference/continue-mcp).

## Using JSON MCP Format from Claude, Cursor, Cline, etc

<Info>
If you're coming from another tool that uses JSON MCP format configuration files (like Claude Desktop, Cursor, or Cline), you can copy those JSON config files directly into your `.continue/mcpServers/` directory (note the plural "Servers") and Open Circuit will automatically pick them up.
