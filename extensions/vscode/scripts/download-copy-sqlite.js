...
async function downloadSqlite(target, targetDir) {
  const downloadUrl =
    ...
    target === "win32-arm64"
      ? "https://ocircuit-server-binaries.s3.us-west-1.amazonaws.com/win32-arm64/node_sqlite3.tar.gz"
...
async function installAndCopyEsbuild(target) {
  ...
  await downloadFile(
    `https://ocircuit-server-binaries.s3.us-west-1.amazonaws.com/${target}/esbuild.zip`,
