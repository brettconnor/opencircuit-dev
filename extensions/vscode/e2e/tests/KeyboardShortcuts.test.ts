...
describe("Keyboard Shortcuts", () => {
  ...
  before(async function () {
    ...
    await GUIActions.moveOCircuitToSidebar(VSBrowser.instance.driver);
  ...
  it("Should correctly undo and redo using keyboard shortcuts when writing a chat message", async () => {
    ...
    await chatInput.sendKeys("OCIRCUIT");
    await TestUtils.waitForTimeout(DEFAULT_TIMEOUT.XS);
    ...
    await chatInput.sendKeys(Key.SHIFT + TestUtils.osControlKey + "z");
    await driver.wait(
      until.elementTextIs(chatInput, "HELLO WORLD HELLO OCIRCUIT"),
      DEFAULT_TIMEOUT.SM,
    );
  ...
  // the below 2 skips are skipped because some behaviour from https://github.com/open-circuit-dev/open-circuit/pull/6711 was reverted
