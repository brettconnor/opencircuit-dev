...
The Open Circuit CLI supports running in TTY-less environments (environments without a terminal/TTY), which is essential for:
...
const command = 'oc -p "Analyze the current git diff"';
...
docker run --rm my-image oc -p "Generate a README"
```

### From CI/CD Pipeline

```yaml
- name: Run Open Circuit CLI
  run: |
    oc -p "Review code changes" --format json
```
...
oc -p "Generate commit message for current changes" --silent
```

## Environment Variables

- `FORCE_NO_TTY`: Forces TTY-less mode, prevents stdin reading
- `OCIRCUIT_CLI_TEST`: Marks test environment, prevents stdin reading
...
Usage examples:
  oc -p "please review my current git diff"
  echo "hello" | oc -p
  oc -p "analyze the code in src/"
  oc -p --agent my-org/my-agent
```
...
```bash
oc -p "your prompt" --config config.yaml
```
