...
4. Permissions in `~/.ocircuit/permissions.yaml`
5. Default policies
...
oc --allow Read --ask Write --exclude Bash

# Start in plan mode (read-only tools only)
oc --readonly "Help me understand this codebase"

# Use mode switching during chat
oc "Let me work on this feature"  # Starts in normal mode
...
## `~/.ocircuit/permissions.yaml` (personal settings)
...
```yaml title="~/.ocircuit/permissions.yaml"
allow:
  - Read(*)
...
oc -p --allow write_file "Write a hello world script"

# Headless mode with wildcard permission (allow all tools)
oc -p --allow "*" "Write and run a script"

# Headless mode with specific restrictions
oc -p --exclude run_terminal_command "Clean up the codebase"
```
