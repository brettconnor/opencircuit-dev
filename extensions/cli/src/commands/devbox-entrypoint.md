...
- **Environment persistence:** The devbox entrypoint (control-plane) writes all env vars to `~/.ocircuit/devbox-env` and sources it before `cn serve`, so keys survive suspend/resume. The CLI assumes env is already present.
