 ...
 * 3. Local .ocircuit/agents/*.md and .ocircuit/checks/*.md (fallback)
 */
export async function resolveReviews(
  ...
  // Source 3: Local .ocircuit/agents/*.md and .ocircuit/checks/*.md
...
/**
 * Resolve reviews from local .ocircuit/agents/*.md and .ocircuit/checks/*.md files.
...
function resolveFromLocal(): ResolvedReview[] {
  ...
  const dirs = [
    path.join(cwd, ".ocircuit", "agents"),
    path.join(cwd, ".ocircuit", "checks"),
  ];
