...
models:
  - model: gpt-4
    ...
    expect(output).toContain('oc -p "please review my current git diff"');
    expect(output).toContain('echo "hello" | oc -p');
    expect(output).toContain('oc -p "analyze the code in src/"');
  });
