...
describe("TipsDisplay", () => {
  ...
  it("renders a random tip from the tips array", () => {
    ...
    const expectedTipIndex = Math.floor(0.5 * OCIRCUIT_CLI_TIPS.length);
    const expectedTip = OCIRCUIT_CLI_TIPS[expectedTipIndex];
  ...
  it("always renders the tip structure correctly", () => {
    ...
    const containsAtLeastOneTip = OCIRCUIT_CLI_TIPS.some(
...
describe("OCIRCUIT_CLI_TIPS", () => {
  it("contains at least 6 tips", () => {
    expect(OCIRCUIT_CLI_TIPS.length).toBeGreaterThanOrEqual(6);
  });

  it("all tips are non-empty strings", () => {
    OCIRCUIT_CLI_TIPS.forEach((tip, index) => {
  ...
  it("tips are helpful and informative", () => {
    ...
    const allTipsText = OCIRCUIT_CLI_TIPS.join(" ").toLowerCase();
