...
For personal use you can set the secret in the hub at //settings/secrets or pass it to the CLI environment.
Org-level secrets can only be used for MCP by Background Agents (//hub/agents/overview) when \"Include in Env\" is enabled for the secret.`;
        if (this.isHeadless) {
  ...
  private async getConnectedClient(
    ...
    const client = new Client(
      { name: "ocircuit-cli-client", version: "1.0.0" },
      { capabilities: {} },
    );
