...
This is a CLI tool for Open Circuit that provides an interactive AI-assisted development experience. The architecture consists of:
...
7. **Hooks System** (`src/hooks/`): Event interception system for extending CLI behavior
   ...
   - **Config locations** (lowest to highest precedence):
     - `~/.claude/settings.json`, `~/.ocircuit/settings.json` (user-global)
     - `.claude/settings.json`, `.ocircuit/settings.json` (project)
     - `.claude/settings.local.json`, `.ocircuit/settings.local.json` (project-local)
