import { ConfigJson } from "@opencircuit/config-types";
...
function convertContext(configJson: ConfigJson): ContextYaml[] {
  const context: ContextYaml[] =
    configJson.contextProviders?.map((ctx) => {
      ...
      return {
        uses: `opencircuit-dev/${ctx.name === "open" ? "open-files" : ctx.name}-context`,
        with: ctx.params,
      };
    }) ?? [];
...
function convertCustomCommand(
  ...
  return {
    ...
    prompt: (cmd as any).prompt, // The type is wrong in @opencircuit/config-types
  };
}
